...
> 이 디렉터리는 PCssak Gongyu `v0.2.4` 무료 Early Access의 공개 저장소 계약입니다.
> 버전 고정 GitHub 릴리스와 검증된 자산 9종을 실제 게시 기록으로 봅니다.

PCssak Gongyu는 사용자가 지시한 Windows SMB 공유 폴더 설정, LAN 점검, 네트워크 드라이브
관리와 별도로 동의한 SSH 설정을 돕습니다. `0.2.4`는 **무료 Early Access**이며, 이는 제품
성숙도를 정직하게 표시하는 이름이지 이후 버전도 계속 무료라는 약속은 아닙니다.

## 공식 다운로드와 자동 업데이트

공개 후에는 [GitHub v0.2.4 공식 릴리스](https://github.com/pcssakinc/pcssak-gongyu-releases/releases/tag/v0.2.4)
...
정상 법률 동의 기록이 남은 **0.2.0·0.2.1·0.2.2·0.2.3 사용자는 앱 안에서 0.2.4를 확인하고 다운로드·검증·
...
0.1.1~0.2.3은 설정·공유 장부·SSH 설정·중단 기록을 지우거나 기존 앱을 먼저 제거하지
...
0.2.4는 Windows 스토어 앱의 수신 허용 규칙을 TCP 22 위험 규칙으로 오판해 SSH 켜기가
실패하던 원인을 고칩니다. 유효 정책과 스토어 앱 정책을 함께 읽고, 관측한 규칙 개수·활성 상태·
프로그램이 모두 대응하며 모든 대응 규칙이 프로그램 없는 패키지 전용일 때만 감사에서 제외합니다.
보호 규칙·기준선 기록·OpenSSH 설치 전에 외부 허용 규칙의 충돌 규모를 읽기 전용으로 확인합니다.
공용 물리 LAN에서는 동의한 개인 네트워크 전환이 먼저 실행될 수 있으며, 임시 비활성화 상한을
넘으면 SSH 변경을 시작하지 않고 전환 원복을 시도합니다. 원복이 검증되지 않으면 실패와 복구
기록을 유지합니다. 켜기를 막는 규칙의 개수와 이름 표본은 화면에 표시합니다.
진행 중 작업의 「안전하게 중지」는 작업을 강제로 끊지 않고 안전한 지점에서 원복을 시도하며,
원복이 검증된 경우에만 취소로 기록합니다. 검증하지 못하면 중단 복구 안내를 유지합니다.
OpenSSH 설치는 끝날 때까지 기다리며, 끄기·완전 원복은 첫 변경 전까지만 중지하고 마지막
검증 단계에 들어간 작업은 완료될 수 있습니다. 중단 기록은 앱이 읽기 전용으로 점검해 완전히
꺼진 상태나 계약대로 완료된 상태만 자동
해제하며, 부분 상태는 기존 확인 절차를 유지합니다. 0.2.3의 보호 교체 허용도 설치기의
네 진입 분기에 추가합니다. SSH 끄기는 서비스 중지·사용 안 함을 확인하고 OpenSSH를
제거하지 않습니다. 상세 변경과 검증 범위는
[릴리스 노트](docs/RELEASE-NOTES-v0.2.4.md)를 확인하세요.
...
릴리스에는 설치본 `PCssak-Gongyu-0.2.4-Windows-x64-Setup.exe.sig`도 포함됩니다. 앱은
