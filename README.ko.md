...
> 이 디렉터리는 PCssak Gongyu `v0.3.2` 무료 Early Access의 공개 저장소 계약입니다.
...
## 현재 알려진 SSH 문제 — 2026-09-15

21:02 KST 사용자 실기에서 공개 0.3.2가 기존 SSH 설치를 인식한 뒤 방화벽 규칙 처리
84%에서 `native_method_output_null`, `HRESULT=0`으로 실패했습니다. 실패 후 원복 검증도
완료되지 않았고 다시 켜기를 시도하면 2%에서 `LEGACY-RECOVERY`로 중단됐습니다.
HRESULT가 0이라는 사실만으로 작업이나 원복 성공을 판단할 수 없습니다.

현재 원인을 조사하고 수정 중이며 해당 PC의 해결은 확인하지 못했습니다. 로컬 자동 시험과
공개 자산 검증 통과는 이 실기 실패의 해결 증거가 아닙니다. 홈페이지의 0.3.2 배포는
보류했고 현재 홈페이지는 0.2.8을 유지합니다. 이는 구버전으로 되돌리라는 권고가 아닙니다.
같은 문제가 발생하면 SSH 켜기·복구를 반복하지 말고 앱과 복구 기록을 보존한 채
[현재 지원 안내](SUPPORT.md)를 확인하세요. 차단을 우회하려고 복구 기록을 삭제하거나
방화벽 규칙을 수동 초기화하지 마세요.
...
정상 법률 동의 기록이 남은 **0.2.0·0.2.1·0.2.2·0.2.3·0.2.4·0.2.5·0.2.6·0.2.7·0.2.8 및 비공개 0.2.9·0.3.0·0.3.1 사용자는 앱 안에서 0.3.2 버전을 확인하고 다운로드·검증·
...
0.1.1~0.3.1은 설정·공유 장부·SSH 설정·중단 기록을 지우거나 기존 앱을 먼저 제거하지
...
상세 변경과 검증 범위는 [릴리스 노트](docs/RELEASE-NOTES-v0.3.2.md)를 확인하세요.
...
릴리스에는 설치본 `PCssak-Gongyu-0.3.2-Windows-x64-Setup.exe.sig`도 포함됩니다. 앱은
