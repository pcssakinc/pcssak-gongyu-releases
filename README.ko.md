...
> 이 디렉터리는 PCssak Gongyu `v0.1.7` 무료 Early Access의 공개 저장소 계약입니다.
> 버전 고정 GitHub 릴리스와 검증된 자산 9종을 실제 게시 기록으로 봅니다.

PCssak Gongyu는 사용자가 지시한 Windows SMB 공유 폴더 설정, LAN 점검, 네트워크 드라이브
관리와 별도로 동의한 SSH 설정을 돕습니다. `0.1.7`은 **무료 Early Access**이며, 이는 제품
성숙도를 정직하게 표시하는 이름이지 이후 버전도 계속 무료라는 약속은 아닙니다.

## 공식 다운로드와 자동 업데이트

공개 후에는 [GitHub v0.1.7 공식 릴리스](https://github.com/pcssakinc/pcssak-gongyu-releases/releases/tag/v0.1.7)
...
공개 `0.1.0`에는 앱 내 자동 업데이트가 없습니다. 해당 사용자는 위 공식 `0.1.7` 설치본을
...
`0.1.7`은 6자리 PIN 앱 잠금, 현재 사용자 바탕화면·문서·다운로드 등의 전용 하위 폴더 공유,
일반 외부 Windows 공유 등록 해제와 페어링·중단된 작업 안내 개선을 포함합니다. 외부 공유를
해제해도 원본 파일과 권한은 유지합니다. 변경 내용과 제한은 [릴리스 노트](docs/RELEASE-NOTES-v0.1.7.md)를
...
릴리스에는 설치본 `PCssak-Gongyu-0.1.7-Windows-x64-Setup.exe.sig`도 포함됩니다. 앱은
