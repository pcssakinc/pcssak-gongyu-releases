...
저장소는 현재 비공개 준비 상태이며 공개 Release·태그·설치 자산은 아직 없다. 아래 조건을 모두
통과하기 전에는 저장소 공개 전환이나 Release 게시를 수행하지 않는다.
...
다음 중 하나라도 남으면 저장소 공개 전환·자산 업로드·릴리스 게시를 중단한다.
...
- 같은 LAN 두 PC의 SMB·매핑·페어링·SSH 로그인·명령·SFTP·SSH 끄기 실기 증거 미완료
