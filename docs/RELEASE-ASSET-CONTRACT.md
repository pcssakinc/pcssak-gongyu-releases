...
이 문서는 `pcssakinc/pcssak-gongyu-releases`의 무료 `v0.1.5` Early Access에 쓰는
...
공개 `0.1.0`에는 앱 내 updater가 없으므로 해당 사용자는 공식 `0.1.5` 설치본을 한 번 수동으로
...
`v0.1.5` Release에는 아래 아홉 파일만 정확히 올린다.

1. `DOWNLOAD-METADATA.json`
2. `PCssak-Gongyu-0.1.5-MPL-Sources.zip`
3. `PCssak-Gongyu-0.1.5-Windows-x64-Setup.exe`
4. `PCssak-Gongyu-0.1.5-Windows-x64-Setup.exe.sig`
...
`releases/download/v0.1.5/...`처럼 버전이 고정되어야 한다. Windows x86 또는 계약 밖
플랫폼 키는 허용하지 않는다.
...
- `installer`: `PCssak-Gongyu-0.1.5-Windows-x64-Setup.exe`
...
2. 태그와 Release가 모두 없으면 `v0.1.5`를 `draft=true`, `prerelease=false`, Free Early
...
- `draft=false`, `prerelease=false`, `Latest=true`, `v0.1.5` 또는 정확한 9자산 불일치
