...
0.3.2는 2026-09-15T11:56:37Z에 불변 일반 Latest로 공개됐다. Release ID는
`389103774`, 공개 태그 대상은 `e39a3cfb5fced6ad165864717d9f2cb1916d3de7`이다.
최종 소스 14단계와 기존 키의 두 서명·정확한 아홉 자산·공개 후 판독을 통과했다.
직전 공개 0.2.8과 비공개 0.2.9·0.3.0·0.3.1의 기록을 구분하며 아래 과거 명명·일반
계약은 보존한다. 후속 문서 main SHA는 불변 공개 태그 대상이나 빌드 소스가 아니다.

- 설치본: `PCssak-Gongyu-0.3.2-Windows-x64-Setup.exe`
- MPL 원본 묶음: `PCssak-Gongyu-0.3.2-MPL-Sources.zip`
- 설치본 서명: `PCssak-Gongyu-0.3.2-Windows-x64-Setup.exe.sig`

설치본 크기·해시·릴리스 ID·게시 시각은 실제 산출물과 공개 응답으로 결속하며 추정하지 않는다.
이전 절은 당시 후보 기록이다. 법률·공개키와 아홉 자산의 일반 계약을 유지한다.

## 이전 0.3.1 비공개 후보의 명명 범위

당시 개발 후보는 0.3.1이었다. 실제 시험·기존 키의 두 서명·아홉 자산 검증을 통과한 뒤
공개하는 조건이며, 현재 공개 0.2.8과 비공개 0.2.9·0.3.0의 기존 자산을 보존한다.

- 설치본: `PCssak-Gongyu-0.3.1-Windows-x64-Setup.exe`
- MPL 원본 묶음: `PCssak-Gongyu-0.3.1-MPL-Sources.zip`
- 공개 시 설치본 서명: `PCssak-Gongyu-0.3.1-Windows-x64-Setup.exe.sig`

무서명 시험본은 수동 설치용이며 공개 업데이트 자산이 아니다. 아래 0.2.8 공개 계약의
아홉 자산·불변 URL·기존 공개키·법률 정본 검증은 계속 적용한다. 버전 예약은 게시 완료가 아니다.

## 0.2.9 비공개 로컬 시험본의 명명 범위

당시 비공개 시험 버전은 사용자 소유 PC에서만 확인하는 0.2.9다. 해당 작업은 공개 릴리스·
홈페이지·Latest를 갱신하지 않으며, 아래 0.2.8 공개 계약과 기존 자산을 보존한다.
현재 버전에도 제품 파일명과 공개 서명 계약을 혼동하지 않도록 다음 이름을 예약한다.

- 설치본 정규 이름: `PCssak-Gongyu-0.2.9-Windows-x64-Setup.exe`
- 정확한 버전의 MPL 원본 묶음 이름: `PCssak-Gongyu-0.2.9-MPL-Sources.zip`
- 공개 시에만 필요한 설치본 서명 이름: `PCssak-Gongyu-0.2.9-Windows-x64-Setup.exe.sig`

이는 파일의 생성·서명·게시 완료 목록이 아니다. 비공개 시험은 기존 공개키를 보존한
`--no-sign` 수동 설치 경로이며, 시험본 사본에는 비공개 시험 전용임을 표시한다.
서명 파일과 공개 자동업데이트 메타데이터는 만들지 않는다. 공개에 필요한 두 서명과
아래 아홉 자산의 검증 기준은 계속 유지하며, 이 시험본을 공개 후보로 간주하지 않는다.

## 0.2.8 공개 버전의 일반 계약

이 문서는 `pcssakinc/pcssak-gongyu-releases`의 무료 `v0.2.8` Early Access에 쓰는
...
0.2.8은 0.2.0의 법률 정본·업데이트 공개키를 그대로 사용한다. 정상 동의 기록이 있는
0.2.0·0.2.1·0.2.2·0.2.3·0.2.4·0.2.5·0.2.6·0.2.7은 앱에서 다운로드·무결성 확인·사용자 승인 뒤 적용한다. 0.1.9는 현행 법률 문서
확인을 위해 대화형 설치하고 0.1.8 이하는 이전 공개키 전환 때문에 공식 설치본을 한 번
직접 실행한다. 구형 앱의 서명 오류를 우회하지 않는다. 0.1.1~0.2.7은 기존 앱을 먼저
제거하거나 설정·공유·SSH·중단 기록을 지우지 않고 보호 교체한다. 이미 제거한 경우에도
Windows 복구 근거를 임의로 삭제하지 않는다. 0.1.0만 별도 제거 후 설치한다.
법률 정본이 다시 바뀌면 대화형 설치로 안내한다. 기존 공개 태그·9자산·서명·노트는 바꾸지 않는다.
...
`v0.2.8` Release에는 아래 아홉 파일만 정확히 올린다.

1. `DOWNLOAD-METADATA.json`
2. `PCssak-Gongyu-0.2.8-MPL-Sources.zip`
3. `PCssak-Gongyu-0.2.8-Windows-x64-Setup.exe`
4. `PCssak-Gongyu-0.2.8-Windows-x64-Setup.exe.sig`
...
`-PublicEarlyAccess`로 공개하는 0.1.2 이상 0.4.9 이하의 정확한 3요소 Early Access 버전은 설치기와
...
경고하거나 실행을 차단할 수 있음을 명시한다. 이 유예는 0.4.9 초과에서 자동 만료한다.

`-PublicEarlyAccess`가 없는 엄격 경로와 0.4.9 초과에서는 설치본과 내부 앱이 Windows 신뢰
...
Windows 11 Smart App Control 차단은 설치 폴더 준비 오류와 별도로 판단한다. 이번 패치는
SAC·조직 정책을 자동 해제하거나 개별 앱 우회를 제공하지 않는다. SAC 재활성화 가능 여부를
모든 Windows 환경에서 보장하지 않으며 최신 공식 근거와 시험 보류 안내는 `SECURITY.md`를 따른다.
...
`releases/download/v0.2.8/...`처럼 버전이 고정되어야 한다. Windows x86 또는 계약 밖
플랫폼 키는 허용하지 않는다.
...
- `installer`: `PCssak-Gongyu-0.2.8-Windows-x64-Setup.exe`
...
3. 엄격 경로에서는 스크립트 자체의 사전/사후 검사로 완결할 수 없는 세 공급망 경계와 후보별
   ...
   없으면 실패 폐쇄한다. 공개 Early Access `-PublicEarlyAccess` 경로에서는 이 외부 증거를 완료로 가장하지
   않고 0.4.9 초과 공개 전 필수 작업으로 기록하며 입력 자체를 허용하지 않는다.
...
### 공개 Early Access 간소화 정책

관리자 승인에 따라 0.1.2 이상 0.4.9 이하의 정확한 3요소 Early Access 버전은 기능 실측을 위한 공개
무료 Early Access 경로를 사용한다. 최종 소스의 GitHub Actions 또는 승인된 로컬 Windows 검증, Tauri 업데이트 서명, 독립
...
고지하고 0.4.9 초과 버전 공개 전에 엄격 게이트로 복원한다. 스크립트는 0.4.9 초과에서 이 경로를 자동
거부하며, 조립기나 게시기가 미완료 항목을 승인으로 바꾸지 않는다.

반복 배포는 같은 최종 소스 커밋·파이프라인 파일 해시·산출물 해시에 결속한 외부 상태 파일로
재개한다. 검증이 끝난 동일 바이트를 다시 빌드하거나 재서명하지 않는다. Actions를 선택했을
때만 최종 소스 SHA에 대해 한 번 디스패치하며 응답이 불확실하면 기존 실행을 먼저 조회한다.

2026-09-08 승인되고 2026-09-12 범위가 연장된 로컬 경로는 `scripts/verify-local-release.ps1` 또는 단일 진입점의
`-Stage RunLocalVerification`으로 실제 Windows 최종 검증을 수행한다. x64·i686 시험과
Clippy, fmt·i686 check·감사·릴리스 빌드·정적·UI·MPL·고지·릴리스 계약을 포함한다.
`LOCAL-VERIFICATION.json`은 최종 소스·고정 명령 계획·실제 도구 버전과 해시·종료 코드·로그
해시를 결속한다. 같은 소스·계획·도구·로그로 확인되는 성공 단계만 재사용하며 실패·중단된
단계부터 재개한다. 원본 로그와 증거는 공개 자산이나 Git에 넣지 않는다.

게시할 때는 `-PublicEarlyAccess -LocalVerificationEvidencePath <LOCAL-VERIFICATION.json>
-ApprovedLocalVerificationEvidenceSha256 <해시>`를 지정하고 Actions ID·attempt를 생략한다.
게시기는 Draft 생성 전과 공개 직전에 증거·로그·도구·소스를 다시 검증한다. Actions 경로와
로컬 경로를 혼합하지 않으며 건너뛰거나 실패한 Actions를 성공으로 바꾸지 않는다.
로컬 대체는 `0.1.2 <= 버전 <= 0.4.9`에서만 허용되고 0.4.9 초과부터 자동 차단된다. 두 경로 모두
실제 설치·두 PC SSH/SFTP 실기를 대신하지 않으며 미완료 행렬은 공개 고지한다.
...
1. 공식 GitHub CLI 2.96.0의 고정 SHA-256, `GitHub, Inc.` Valid Authenticode와 인증·고정
   ...
   공개 Early Access `-PublicEarlyAccess`에서는 이 두 증거가 미완료임을 승인 노트에 고지하고 해당 인수를
   받지 않는다. 게시기는 실제 승인 증거를 생성하거나 완료로 추정하지 않는다.
2. 태그와 Release가 모두 없으면 `v0.2.8`을 `draft=true`, `prerelease=false`, Free Early
...
- `draft=false`, `prerelease=false`, `Latest=true`, `v0.2.8` 또는 정확한 9자산 불일치
...
- 설치본·MPL·서명 입력의 PE 형식 또는 재분석 지점 검증 실패, 엄격 경로의 Authenticode
  Valid·Code Signing EKU·타임스탬프 계약 실패, 공개 Early Access `-PublicEarlyAccess` 경로에서 설치기와
...
못하면 원격 API를 호출하기 전에 공개를 차단한다. 공개 Early Access `-PublicEarlyAccess`에서는 이 행렬이
미완료임을 릴리스 노트에 공개하고 0.4.9 초과 공개 전 필수 작업으로 남긴다. 어느 경로에서도 모든 백신·
